const fs = require('fs'),
    ...
    // sharp = require('sharp'),
    keptEvents = [
        "SharePresentationEvent", "GotoSlideEvent", "StartDeskshareEvent",
        "DeskShareStartRTMP", "DeskShareStopRTMP"
    ],
    prompt_attributes = [{
        name: 'path',
        ...
        name: 'chat',
        description: "type yes to output chat as subtitle. (default:yes)",
        default: "yes"
    }, {
        name: 'cache',
        description: "Overwrite cache if exist? (default:no)",
        default: "no"
    }, {
        name: 'webcam',
        description: "Should webcam footage be included? (default:no)",
        default: "no"
    }],
...
if (myArgs.length === 0) {
    ...
    prompt.get(prompt_attributes, (err, result) => {
        if (err)
            ...
            let chat = result.chat.toLowerCase() === "yes",
                path = result.path.toString() === "" ? "C:\\Users\\seven\\Desktop" :
                result.path,
    ...
    while (i < myArgs.length) {
        ...
        switch (it) {
            case '-p': {
                ...
                path = myArgs[i + 1].toString() === "" ? "C:\\Users\\seven\\Desktop" :
                    myArgs[i + 1].toString();
                i++;
                break;
            }
            ...
            case '-o': {
                ...
                output_path =
                    myArgs[i + 1].toString() === "" ? "" : myArgs[i + 1].toString();
    ...
    if (required) {
        if (invalid_ouput)
            ...
            main(chat, path, cache, webcam, output_path)
...
async function main(chat, path, cache, webcam, output) {
    ...
    if (chat)
        keptEvents.push("PublicChatEvent");
    ...
    let webcamvideo =
        test.indexOf("data-path=\"video\">../meetingFiles/") > 0 ?
        test.match(
            /"video">..\/meetingFiles\/(?<video>[.A-Z_0-9a-z]*)<\/script>/i)
        .groups.video :
        "";
    let screenvideo =
        test.indexOf("slave-video") > 0 ?
        test.match(
            /"slave-video">..\/meetingFiles\/(?<video>[.A-Z_0-9a-z]*)<\/script>/i)
        .groups.video :
        "";

    console.log(`Video's have been found:\n - ${webcamvideo}\n - ${screenvideo}`);
    // read events
    let json = (await parser.parseStringPromise(
            fs.readFileSync(path + '/meetingFiles/events.xml')))
        .recording;
    ...
    if (gpu_enabled) {
        ...
        await (new Promise(
            (resolve, reject) =>
            videos
            .reduce((result, input) => result.input(input), FfmpegCommand())
            .format('mp4')
            .on('end', resolve)
            .on('error', reject)
            .on('stderr',
    ...
    if (!webcam) {
        ...
        console.error(
            "Not yet implemented, pls run again without webcam/overwriting cache")
    }
}
...
async function handle_events(events, path, chat, name) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        switch (item['$'].eventname) {
            case "SharePresentationEvent": {
                picture =
                    path + "/meetingFiles/presentation/" + item.presentationName + "/";
                let image = picture + "slide-1.png";
                let time = find_next(events, i,
                        [
                            "GotoSlideEvent", "StartDeskshareEvent",
            ...
            case "GotoSlideEvent": {
                ...
                /* resizing images               res = await fs.readFile(image, async
                   (err, data) => { if (err) throw err; res = await
                   sharp(data).resize(1600, 1200, { fit: 'fill',
                                    }).toFile(image, () => console.log("resized image: " +
                   image));
                                });
                */
                let time = find_next(events, i,
            ...
            case "DeskShareStartRTMP": {
                ...
                let time =
                    find_next(events, i, ["StartDeskshareEvent"]) - item['$'].timestamp;
                images.push({
                    path: image,
                    loop: 1,
                    video_ignored: false,
            ...
            case "StartDeskshareEvent": {
                ...
                let time =
                    find_next(events, i, ["DeskShareStopRTMP"]) - item['$'].timestamp;
                images.push({
                    path: path,
                    video_ignored: true,
                    ignored_loop_param: time,
            ...
            case "PublicChatEvent": {
                if (chat) {
                    ...
                    chatmessages.push(parse_to_srt_timestamp(item['$'].timestamp -
                        events[0]['$'].timestamp));
    ...
    bar1.stop();
    if (chat)
        fs.writeFileSync(`${path}/${name}.srt`, chatmessages.join("\n"));
    return images;
}
...
function parse_to_srt_timestamp(time) {
    ...
    let min = Math.floor(time / 60) % 60,
        hour = Math.floor(time / 3600) % 3600,
        seconds = Math.floor(time % 60);
    return `${hour}:${min}:${seconds},${milli} --> ${hour}:${min}:${
      seconds + 3},${milli}`;
}
            ...
            if (cache || !fs.existsSync(`${path}/temp/finaltempp${i}.mp4`)) {
                if (!images[i].video_ignored) {
                    ...
                    if (loop < 1) {
                        ...
                        await images_to_video(images[i], i,
                            `${path}/temp/finaltempp${i}.mp4`);
                    } else {
                        test = images[i];
                        loop -= 1;
                    ...
                    let duration = (images[i].ignored_loop_param - 1000) / 1000,
                        start = images[i].start / 1000;
                    let min = Math.floor(start / 60) % 60,
                        hour = Math.floor(start / 3600) % 3600,
                        seconds = Math.floor(start % 60);
                    let tmin = Math.floor(duration / 60) % 60,
                        thour = Math.floor(duration / 3600) % 3600,
...
function find_next(events, k, find) {
    for (let i = k + 1; i < events.length; i++)
        if (find.includes(events[i]['$'].eventname))
